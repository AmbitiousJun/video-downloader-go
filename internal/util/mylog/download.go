...
func printDownloadLogs() {
	...
	validWidth := width - SuccessLogPrefixSize
	...
	logs = append(logs, PackMsg("SUCCESS", ANSISuccess, fmt.Sprintf("%s%s%s", strings.Repeat("·", phLeftSize), DlLogStart, strings.Repeat("·", phRightSize))))
	...
	for _, log := range downloadLogs {
		...
		if i != len(downloadLogs)-1 {
			logs = append(logs, PackMsg("SUCCESS", ANSISuccess, strings.Repeat("·", validWidth)))
		}
		i++
	}

	// 输出结束日志
	logs = append(logs, PackMsg("SUCCESS", ANSISuccess, fmt.Sprintf("%s%s%s", strings.Repeat("·", phLeftSize), DlLogEnd, strings.Repeat("·", phRightSize))))
	...
	if (lastQueueLog == nil && downloadLogFlag.Load()) || (lastQueueLog != nil && isDownloadLogEnd(lastQueueLog)) {
		clearLog := PackMsg("SUCCESS", ANSISuccess, fmt.Sprintf("\033[%dF\033[J\r", totalLogLines+1))
		logs = append([]*logItem{clearLog}, logs...)
	} else {
		emptyLog := PackMsg("SUCCESS", ANSISuccess, "")
